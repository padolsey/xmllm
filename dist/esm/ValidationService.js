...
var ValidationService = /*#__PURE__*/function () {
  ...
  return _createClass(ValidationService, null, [{
    ...
    value: function validateMessages(messages) {
      ...
      if (messages.length > 0 && messages[0].role === 'system') {
        ...
        if (typeof messages[0].content !== 'string') {
          throw new MessageValidationError('System message content must be a string', {
      ...
      messages.forEach(function (message, index) {
        if (!message.role || !message.content) {
          throw new MessageValidationError('Message must have role and content', {
            index: index,
            message: message
          });
        }
        if (!['user', 'assistant', 'developer'].includes(message.role)) {
          throw new MessageValidationError('Invalid message role', {
            index: index,
            role: message.role,
        ...
        if (typeof message.content !== 'string') {
          throw new MessageValidationError('Message content must be a string', {
            index: index,
    ...
    value: function validateSingleModel(model, availableModels) {
      ...
      if (typeof model !== 'string') {
        if (!(model !== null && model !== void 0 && model.name)) {
          throw new ModelValidationError('Custom model must me object with a name', {
            model: model,
            index: index
          });
        }
      ...
      var _model = model || '';
      var _colon = _model.indexOf(':');
      var provider = _colon === -1 ? _model : _model.slice(0, _colon);
      var type = _colon === -1 ? undefined : _model.slice(_colon + 1);
      ...
      if (!availableModels[provider]) {
        throw new ModelValidationError('Provider not found', {
          provider: provider,
          index: index,
    ...
    value: function validateConstraints(constraints) {
      ...
      if (constraints.rpmLimit !== undefined) {
        ...
        if (typeof constraints.rpmLimit !== 'number' || Number.isNaN(constraints.rpmLimit)) {
          throw new PayloadValidationError('rpmLimit must be a whole number', {
        ...
        if (!Number.isFinite(constraints.rpmLimit)) {
          throw new PayloadValidationError('rpmLimit must be finite', {
            rpmLimit: constraints.rpmLimit
          });
        }
        ...
        if (constraints.rpmLimit <= 0) {
          throw new PayloadValidationError('rpmLimit must be positive', {
            rpmLimit: constraints.rpmLimit
          });
        }
      }
    ...
    value: function validateLLMPayload() {
      ...
      if (strategy && !schema) {
        throw new PayloadValidationError('Strategy can only be used with schema-based prompts. For raw prompts, use system/messages directly.', 'STRATEGY_VALIDATION_ERROR', {
          strategy: strategy
        });
      }
      ...
      if (hints && !schema) {
        throw new PayloadValidationError('Cannot provide hints without a schema', 'HINT_VALIDATION_ERROR', {
          hints: hints
        });
      }
      ...
      if (temperature !== undefined) {
        if (typeof temperature !== 'number' || temperature < 0 || temperature > 1) {
          throw new PayloadValidationError('Temperature must be between 0 and 1', {
      ...
      if (max_tokens !== undefined) {
        if (!Number.isInteger(max_tokens) || max_tokens <= 0) {
          throw new PayloadValidationError('max_tokens must be a positive integer', {
            max_tokens: max_tokens
          });
        }
      }
      if (stream !== undefined && typeof stream !== 'boolean') {
        throw new PayloadValidationError('stream must be a boolean', {
          stream: stream
        });
      }
      if (cache !== undefined) {
