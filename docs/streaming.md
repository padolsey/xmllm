...
xmllm offers four modes for handling streaming data for which you've defined a [schema](./schema.md):
...
## Handling Complete Elements

There are two ways to handle complete vs partial elements in xmllm:

### 1. Schema Mode (When using schemas)
```javascript
// Using schema with mode
const stream1 = stream('List colors', {
  schema: { color: Array(String) },
  mode: 'delta'  // Only yields complete elements
});
// Yields: { color: ['red'] }, { color: ['blue'] }

// Default state mode shows partial updates
const stream2 = stream('List colors', {
  schema: { color: Array(String) }
  // mode: 'state' is default
});
// Yields: { color: ['re'] }, { color: ['red'] }, { color: ['red', 'blu'] }...
```

### 2. closedOnly() (When using select())
```javascript
// Using select with closedOnly
const stream1 = stream('List colors')
  .select('color')
  .closedOnly()  // Only yields complete elements
  .text();
// Yields: 'red', 'blue'

// Without closedOnly shows partial updates
const stream2 = stream('List colors')
  .select('color')
  .text();
// Yields: 're', 'red', 'blu', 'blue'
```

### When to Use Each

- Use **modes** when working with schemas for structured data
- Use **closedOnly()** when using select() for raw XML elements
- They serve similar purposes but at different levels:
  - Modes control how schema-mapped results are delivered
  - closedOnly() filters raw XML elements before mapping